...
def estimate_volatility(prices, l):
    ...
    ##print(l)
    # log return
    log_return = (np.log(prices) - np.log(prices.shift(1)))**2
    #lambda is  "1 - l"
    true_lambda = 1 -l
    # exponentially weighted moving window
    ewmw = log_return.ewm(alpha = true_lambda).mean()
    
    #square root is the volility / standard devitation
    print(np.sqrt(ewmw)[-1])
    # ans required the last volility only
    return np.sqrt(ewmw[-1])
    pass
